...
def get_layout():
    try:
        ...
        >
        As soon as a pickled study is available, you can refresh the page to load the dashboard.
    ...
    summary_fig.add_trace(go.Table(header=dict(values=['Parameter Name', 'Best Value']), cells=dict(values=[trial_param_names, best_values])), 1, 1)
    summary_fig.add_trace(go.Pie(labels=['Completed', 'Pruned'],
    ...
    children = [
        ...
        dash_table.DataTable(id='Trials Table',
                             columns=[{'name': i, 'id': i} for i in trials_dataframe.columns],
    ...
    except AssertionError: # In some cases, the hyperparameter importance plot will fail with this error
        pass
